...
export function FileUploader({ onFileSelect, selectedFile }: FileUploaderProps) {
  ...
  const handleFile = (file: File) => {
    // Check if file is a text file, PDF, or DOCX
    const fileName = file.name.toLowerCase();
    const isSupportedFile = fileName.endsWith('.txt') || 
    ...
    if (!isSupportedFile) {
      alert('Please upload a text file (.txt), PDF, or DOCX file.');
      return;
    }
      ...
      {selectedFile && selectedFile.size > 0 ? (
        ...
        >
          <div className="flex flex-col items-center justify-center pt-5 pb-6">
            ...
            <p className="text-xs text-gray-500">TXT, PDF, or DOCX files only (max 5MB)</p>
          </div>
        </div>
      )}
    </div>
